...
function window_on_load(evt) {
	...
	app = App.new(canvas, settings);

	const dialog = app.create({ type: 'dialog', x: 70, y: 70, w: 100, h: 50, title: 'Dialog box', text: 'Would you like\nto do this?!' });
	dialog.onok = dialog_on_ok;
	dialog.oncancel = dialog_on_cancel;

	app.create({ type: 'text', x: 5, y: 5, text: 'How are you today, Papa?' });

	const button1 = app.create({ type: 'button', x: 5, y: 15, text: 'Good, thank you.' });
	button1.onmousedown = button1_on_mousedown;

	const button2 = app.create({ type: 'button', x: 5, y: 35, text: 'Not very well, thank you.' });
